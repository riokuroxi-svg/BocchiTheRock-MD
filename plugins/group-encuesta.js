let handler = async (m, { conn, text, args, usedPrefix, command }) => {
  ...
  // Validaciones
  if (!args[0]) throw `${emojiBocchi} En las sombras no hay nada...\n${emojiChristmas} Ingresa un texto para iniciar la *Encuesta Navideña del Bocchi*.\n\n📌 Ejemplo:\n*${usedPrefix + command}* Pizza|Hamburguesa|Tacos`
  ...
  for (let i = 0; i < partes.length; i++) {
    opciones.push([`${emojiBocchi} ${partes[i].trim()} ${emojiChristmas}`])
  }

  // Título de la encuesta con estilo Bocchi + Navidad
...
handler.tags = ['grupos']
handler.command = ['poll', 'encuesta', 'Bocchipoll']
handler.group = true
